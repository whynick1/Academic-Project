...
///////////////////////////////////////////////////////
val conf = new SparkConf().setAppName("Problem5_Howie").setMaster("local")
val sc = new SparkContext(conf)


val lines = scala.io.Source.fromFile("/Users/wanghongyi/Applications/spark/data/n_bi_gram.txt").mkString
val arr = lines.toLowerCase().split("\\s+")
...
//val stopwords = List("**Empty**")

var lst = new ListBuffer[String]()
for(i <- 0 until arr.length){
     if (!stopwords.contains(arr(i)) && arr(i) != "") {
          lst += PorterStemmer.stem(arr(i)) + " ";
     }
}
...
printToFile(new File("/Users/wanghongyi/Applications/spark/data/n_bi_gram2.txt")) { p =>
  lst.foreach(p.print)
}

///////////////////////
val rdd = sc.textFile("/Users/wanghongyi/Applications/sparkdata/n_bi_gram2.txt")
