...
//			toPrint = "yes";
		...
		{
			System.out.println("############ Print decision tree of Information gain heuristic ############");
			PrintTree(root, 0);
		}
		double accuracy = id3.computeAccuracy(predictions, testLookUpTable);
		System.out.println("Accuracy with Information gain heuristic is: "
				+ accuracy);
		...
		ID3 id3_2 = new ID3();
		id3_2.learn(lookUpTable, 1);
		//produce predicted result of test_set.csv
		predictions = id3_2.classify(testLookUpTable);
		//print accuracy
		if (toPrint.equals("yes"))
		{
			System.out.println("############ Print decision tree of Variance impurity heuristic ############");
			PrintTree(root, 0);
		}
		accuracy = id3_2.computeAccuracy(predictions, testLookUpTable);
		System.out.println("Accuracy with Variance impurity heuristic is: "
				+ accuracy);
		//parameters in form of (root, attribute_number, L, K)
		pp = new PostPruning(root, ATTRIBUTE_NUMBER, L, K, valiLookUpTable, accuracy);
		pp.pruning(root);
	}
