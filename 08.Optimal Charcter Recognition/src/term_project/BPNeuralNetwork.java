...
{
	private final double[] input;		//input vector
	private final double[] hidden;		//hidden layer
	private final double[] output;		//output layer
	private final double[] target;		//target

	private final double[] hidDelta;	/** delta vector of the hidden layer */
	private final double[] optDelta;	/** output layer of the output layer */

	private final double eta;			//learning rate
	private final double momentum;		//momentum
	...
	/** Constructor.
	 * Note:The capacity of each layer will be the parameter plus 1.
	 * The additional unit is used for smoothness. */
	public BPNeuralNetwork(int inputSize, int hiddenSize, int outputSize, double eta, double momentum) {

		input = new double[inputSize + 1];
		hidden = new double[hiddenSize + 1];
		output = new double[outputSize + 1];
		target = new double[outputSize + 1]; // Length of target is equal to output.
	 ...
	 * @param trainData: ÊÇÒ»¸öÒ»Î¬Êý×é£¬±íÊ¾Ò»¸öÑµÁ·Ñù±¾
	 * @param target£ºÊÇÒ»¸öÒ»Î¬Êý×é£¬Æä³¤¶È¸úÊä³ö²ãµÄÉñ¾­ÔªÊýÁ¿ÏàµÈ
	 */
	 ...
	 * Forward.
	 * @param layer0
	 * @param layer1£º±£´æµÄÊÇsigmoidºóÊä³öµÄ½á¹û
	 * @param weight
	 */
	 ...
	 * hidOptWeights[j][...] : ±íÊ¾Òþ²Ø²ãµÄÒ»¸öµ¥Ôªµ½Êä³ö²ã¸÷¸öµ¥ÔªµÄÈ¨Öµ
	 */
	...
	/**
	 * Adjust the weight matrix in certain layer.
	 */
	private void adjustWeight(double[] delta, double[] layer, double[][] weight, double[][] prevWeight)
	{
		...
		for (int i = 1, len = delta.length; i != len; ++i) {
			for (int j = 0, len2 = layer.length; j != len2; ++j) {
				...
				weight[j][i] += newVal;
				prevWeight[j][i] = newVal; // save this time update
			}
		}
	}
